...
WtIolist::WtIolist(/*int columns*/)
{
    ...
    /*std::unique_ptr<Wt::WStandardItemModel> IoTable =
        Wt::cpp14::make_unique<Wt::WStandardItemModel>(0,5);
    for (int i =0; i<ColumnNames.size(); i++) 
    {
        IoTable->setHeaderData(i, Wt::Orientation::Horizontal, std::string(ColumnNames.at(i)));
    }
    IoTable->insertRow(0,std::move(MakeRow({"Row11","Row12"})));
    // std::cout<<MakeRow({"Row11","Row12"});
    IoTable->insertRow(1,std::move(MakeRow({"Row21","Row22"})));
    setModel(std::move(IoTable));*/
    // addStyleClass("tcontainer");
    // setStyleClass("Wt-itemview Wt-tableview");
    setStyleClass("Wt-table");
    setAttributeValue("style", "table-layout: fixed;");
    setMinimumSize(Wt::WLength(100, Wt::LengthUnit::Percentage),
        Wt::WLength(5, Wt::LengthUnit::Percentage));
    setSelectable(false);
    setHeaderCount(1);
    setMargin(Wt::WLength(1));
    for (int i = 0; i<ColumnNames.size(); i++)
    {
        elementAt(0, i)->addWidget(std::make_unique<Wt::WLabel>(std::string(ColumnNames.at(i))));
    }
    for (int i = 0; i<ColumnNames.size(); i++)
    {
        elementAt(1, i)->addWidget(std::make_unique<Wt::WLabel>(std::string(ColumnNames.at(i))));
        Wt::WLabel* mylabel = (Wt::WLabel*)elementAt(1, i)->widget(0);
        elementAt(1,i)->clicked().connect(std::bind(&WtIolist::ClickEvent,
            this, mylabel->text()));
    }
    for (int i = 0; i<ColumnNames.size(); i++)
    {
        elementAt(2, i)->addWidget(std::make_unique<Wt::WLabel>(std::string(ColumnNames.at(i))));
    }
    for (int i = 0; i<ColumnNames.size(); i++)
    {
        elementAt(3, i)->addWidget(std::make_unique<Wt::WLabel>(std::string(ColumnNames.at(i))));
    }
}

std::vector<std::unique_ptr<Wt::WStandardItem>> WtIolist::MakeRow(
    std::vector<std::string> data)
{
    std::vector<std::unique_ptr<Wt::WStandardItem>> result;
    for (auto it = data.begin(); it != data.end(); ++it) 
    {
        result.push_back(std::move(
            Wt::cpp14::make_unique<Wt::WStandardItem>(Wt::WString(*it))));
    }
    return result;
}

/*void WtIolist::RedrawItems(int min, int max)
{

}*/

void WtIolist::ClickEvent(Wt::WString label)
{
    // elementAt(1, 2)->addWidget(std::make_unique<Wt::WLabel>(label));
    for (int i = 0; i<ColumnNames.size(); i++)
    {
        elementAt(4, i)->addWidget(std::make_unique<Wt::WLabel>(std::string(ColumnNames.at(i))));
    }
}
